...
const ProcessActivities = ({
  ...
  const handleESignConfirm = async ({ email, password, comment }) => {
    if (isSubmitting || !selectedActivity || !recordId) return;
    if (!activityApi) {
      toast.error("Activity API is not configured.");
      return;
    }
    try {
    ...
    //   toast.success(`${selectedActivity.name} completed successfully.`);
    const activity = response?.data?.data?.activity;
      ...
      setIsESignOpen(false);
      setSelectedActivity(null);
      await onActivitySuccess?.(response, selectedActivity);
    } catch (error) {
      console.error("Activity execution failed:", error);
      toast.error(error?.response?.data?.message || error?.message || `Failed to perform ${selectedActivity.name}.`);
    } finally {
    ...
    <>
      <div className="flex min-h-[48px] w-full items-center rounded-xl border border-[#E0EAE5] bg-[#F3F8F5] px-3 shadow-[0_2px_10px_rgba(21,44,32,0.025)]">
        {/* <div className="mr-3 flex shrink-0 items-center gap-1.5 border-r border-[#DCE8E2] pr-3">
          ...
          <span className="hidden text-[10px] font-semibold text-[#60736B] sm:block">Activities</span>
        </div> */}
    ...
    <span className="text-[10px] text-[#8A9992]">
      No actions available for this stage
        ...
        <div className="ml-3 flex shrink-0 items-center gap-2 border-l border-[#DCE8E2] pl-3">
          ...
          >
            <ClipboardCheck size={12} strokeWidth={2} className="transition-transform duration-200 group-hover:scale-105" />
            <span>Audit</span>
          </button>
          <button
            type="button"
          ...
          >
            <LogOut size={12} strokeWidth={2} className="transition-transform duration-200 group-hover:-translate-x-0.5" />
            <span>Exit</span>
          </button>
        </div>
      </div>

      <ESignModal isOpen={isESignOpen} activity={selectedActivity} loading={isSubmitting} onClose={handleCloseESign} onConfirm={handleESignConfirm} />
    </>
  );
};
