...
const SkeletonBox = ({ className = "", style = {} }) => (
  <div className={`${shimmerBase} rounded-lg ${className}`} style={style} />
);
...
const TableCellSkeleton = ({ type = "text", align = "left" }) => {
  ...
  switch (type) {
    case "number":
      return <div className={`flex ${alignment}`}><SkeletonBox className="h-4 w-8 rounded-md" /></div>;
    case "avatar":
      return <div className={`flex ${alignment}`}><SkeletonBox className="h-9 w-9 rounded-full" /></div>;
    case "avatarText":
      return (
    ...
    case "badge":
      return <div className={`flex ${alignment}`}><SkeletonBox className="h-6 w-20 rounded-full" /></div>;
    case "date":
      return <div className={`flex ${alignment}`}><SkeletonBox className="h-3.5 w-24" /></div>;
    case "email":
      return <div className={`flex ${alignment}`}><SkeletonBox className="h-3.5 w-36 max-w-full" /></div>;
    case "actions":
      return (
    ...
    case "action":
      return <div className={`flex ${alignment}`}><SkeletonBox className="h-8 w-20 rounded-lg" /></div>;
    case "text":
    default:
      return <div className={`flex ${alignment}`}><SkeletonBox className="h-3.5 w-28 max-w-full" /></div>;
  }
};
...
const TableSkeleton = ({
  ...
  const defaultColumns = Array.from({ length: columns }).map((_, index) => ({
    type: index === 0 ? "number" : index === columns - 1 ? "actions" : "text",
    width: index === 0 ? "10%" : index === columns - 1 ? "20%" : undefined,
    align: index === 0 ? "center" : index === columns - 1 ? "right" : "left",
  }));

  const tableColumns = columnDefinitions || defaultColumns;

  return (
    <div className={`w-full overflow-hidden rounded-xl border border-[#DDEAE2] bg-white ${className}`}>
      <table className="w-full table-fixed border-collapse">
        ...
        {showHeader && (
          <thead>
            <tr className="border-b border-[#E5EEE9] bg-[#F8FBF9]">
              {tableColumns.map((column, index) => (
                <th key={index} className={`px-5 py-4 text-${column.align || "left"}`}>
                  <SkeletonBox
                    className={
                      column.type === "number"
                        ? "mx-auto h-3.5 w-8"
        ...
        <tbody>
          {Array.from({ length: rows }).map((_, rowIndex) => (
            <tr key={rowIndex} className="border-b border-[#EDF3EF] last:border-b-0">
              {tableColumns.map((column, columnIndex) => (
                <td key={columnIndex} className="px-5 py-4">
                  <TableCellSkeleton type={column.type} align={column.align} />
                </td>
              ))}
            </tr>
          ))}
...
const CardsSkeleton = ({ count = 4 }) => (
  <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
    {Array.from({ length: count }).map((_, index) => (
      <div key={index} className="rounded-2xl border border-[#DDEAE2] bg-white p-5 shadow-[0_8px_25px_-20px_rgba(21,61,45,0.35)]">
...
const ChartSkeleton = ({ height = "h-72" }) => (
  <div className="rounded-2xl border border-[#DDEAE2] bg-white p-5 shadow-[0_8px_25px_-20px_rgba(21,61,45,0.35)]">
    ...
    <div className={`relative mt-7 ${height} overflow-hidden rounded-xl bg-[#F8FBF9]`}>
      <div className="absolute inset-x-5 bottom-5 top-5 flex items-end gap-4">
        {[55, 72, 42, 85, 64, 78, 48, 91, 60, 76, 52, 68].map((barHeight, index) => (
          <SkeletonBox key={index} className="flex-1 rounded-t-md rounded-b-none" style={{ height: `${barHeight}%` }} />
        ))}
      </div>
    </div>
  </div>
...
const Skeleton = ({
  ...
  const skeletons = {
    ...
    table: <TableSkeleton rows={rows} columns={columns} showHeader={showHeader} columnDefinitions={columnDefinitions} />,
  ...
  if (!document.getElementById(styleId)) {
    ...
    style.innerHTML = `
      ...
      @media (prefers-reduced-motion: reduce) {
        .premium-skeleton { animation: none !important; }
      }
    `;
