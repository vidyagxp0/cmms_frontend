...
const UNIT_MASTER = [
  ...
  {
    discipline: "Light intensity",
    units: ["Lumen", "Lux"],
  },
  {
    discipline: "Flow",
    units: ["gm/min", "ml/min", "LPM", "LPH", "m3/Hr"],
  },
  {
    discipline: "Conductivity",
    units: ["µS/cm", "mS/cm"],
  },
  {
    discipline: "pH",
  ...
  {
    discipline: "Rotataion",
    units: ["rpm"],
  },
  {
    discipline: "Elecrical",
    units: ["mV", "V", "mA"],
  },
  {
    discipline: "Time",
  ...
  {
    discipline: "Particle count",
    units: ["μ", "μm"],
  },
  {
    discipline: "Gas analyser",
    units: ["%Vol", "% O2"],
  },
  {
    discipline: "Accuracy",
...
const parseValueAndUnit = (value) => {
  ...
  const matched = allUnits.find(({ unit }) => {
    return text.endsWith(` ${unit}`) || text.endsWith(unit);
  });
  ...
  if (text.endsWith(` ${matched.unit}`)) {
    rawValue = text
      .slice(0, -(` ${matched.unit}`).length)
      .trim();
  } else {
    rawValue = text
      .slice(0, -matched.unit.length)
...
const SymbolicInput = ({
  ...
  const [discipline, setDiscipline] = useState(
    initial.discipline || defaultDiscipline
  );

  const [unit, setUnit] = useState(initial.unit || "");

  const [symbolEnabled, setSymbolEnabled] = useState(
    Boolean(initial.unit)
  );

  const [showSelector, setShowSelector] = useState(false);

  /*
   * Synchronize value when Form.setFieldsValue()
   * receives API/edit data.
   */
  useEffect(() => {
  ...
  /*
   * Automatically select the only unit for a discipline.
   *
   * Temperature -> °C
  ...
  useEffect(() => {
    if (!unit && discipline) {
      const disciplineData = UNIT_MASTER.find(
        (item) => item.discipline === discipline
      );

      if (disciplineData?.units?.length === 1) {
        setUnit(disciplineData.units[0]);
      }
    }
  }, [discipline, unit]);

  /*
   * Final value sent to Ant Design Form.
   *
   * 100
   * 100 °C
   * 25 °C
  ...
  const handleToggle = (checked) => {
    ...
    if (checked) {
      ...
      /*
       * Temperature automatically becomes °C.
       */
      if (!unit && discipline) {
        const disciplineData = UNIT_MASTER.find(
          (item) => item.discipline === discipline
  ...
  const handleTreeSelect = (selectedKeys, info) => {
    ...
    const selectedDiscipline = selectedKey.slice(
      0,
      separatorIndex
    );

    const selectedUnit = selectedKey.slice(
      separatorIndex + 2
    );
  ...
  return (
    <div className={`w-full ${className}`}>
      {/* =====================================================
          ONE SINGLE VALUE INPUT + SYMBOL CONTROL
          ===================================================== */}
  ...
  {symbolEnabled && unit && rawValue && (
    <span
      ...
      "
      style={{
        left: `${Math.max(12, rawValue.length * 8 + 16)}px`,
      }}
    >
      {unit}
    </span>
        ...
        {/* Symbol stays close to input */}
        <div className="flex shrink-0 items-center gap-2">
          <span className="text-sm font-medium text-[#263B35]">
            Symbol
          </span>
      ...
      {/* =====================================================
          SELECTED UNIT
          Just a small clickable label, NOT AN INPUT
          ===================================================== */}
