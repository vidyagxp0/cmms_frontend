...
const Audit = () => {
  ...
  /*
   * Parse attachment audit data regardless of whether
   * backend returns an array, object, JSON string,
   * nested attachments object, or double encoded JSON.
   */
  const parseAttachmentValue = (value) => {
    ...
    /* support multiple levels of JSON encoding */
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      if (
        ...
        if (Array.isArray(currentValue.attachments)) {
          currentValue = currentValue.attachments;
          continue;
        }
  ...
  const getAttachmentUrl = (attachment) => {
    ...
    if (directUrl) {
      ...
      return `${backendUrl}/${directUrl.replace(/^\/+/, "")}`;
    }
  ...
  /*
   * Render attachments as clickable filenames.
   *
   * The raw attachment array is NEVER rendered.
   */
  ...
  /*
   * Format normal audit values.
   *
   * Never return a raw object because React cannot render
   * an object directly as a child.
   */
