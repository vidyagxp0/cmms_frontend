...
const CalibrationGrid = ({
  ...
  const updateRow = (rowIndex, key, newValue) => {
    ...
    if (key === "equipmentInstrumentName") {
      ...
      if (equipmentId && equipmentMap[equipmentId]) {
        ...
        row.equipmentInstrumentId = eq.equipment_id || "";
        row.makeModel = `${eq.make || ""} ${eq.model || ""}`.trim();
      } else {
  ...
  const renderField = (column, row, rowIndex) => {
    ...
    const handleChange = (newValue) => {
      updateRow(rowIndex, key, newValue);
    };
            ...
            {rows.length > 0 ? (
              rows.map((row, rowIndex) => (
                <tr key={rowIndex} className="group bg-white transition-colors duration-150 hover:bg-[#FAFCFB]">
                  ...
                  {visibleColumns.map((column) => (
                    <td key={column.key} className="border-b border-r border-[#E0E7E4] px-3 py-2.5 align-top">
              ...
              <tr>
                <td colSpan={visibleColumns.length + 2} className="h-[20px] border-b border-[#E0E7E4] px-5" />
              </tr>
            )}
          </tbody>
        </table>
