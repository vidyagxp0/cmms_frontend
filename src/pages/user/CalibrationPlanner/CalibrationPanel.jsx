...
import {
    getAllActivites,
    getAllStages,
} from "../../../services/usersApi/workflowApi";
...
const buildProcessData = (values, systemFields) => [
    ...
    },
    {
        key: "qa_reviewer",
        label: "QA Reviewer",
        value: values?.qaReviewer || "",
    },
    {
        key: "qa_approval",
        label: "QA Approval",
        value: values?.qaApproval || "",
    },
    {
        key: "comment",
        label: "Comments",
...
const CreateCalibrationPanel = () => {
    ...
    const fetchCalibrationDetail = useCallback(async () => {
        if (!recordId) {
            toast.error(
                "Calibration record ID is missing."
            );
            return;
        }
        ...
        try {
            ...
            const response =
                await getCalibrationDetail(recordId);
            ...
            // =========================
            setProcessId(
                responseData?.process_id || null
            );

            setActiveStageId(
                responseData?.stage?.id ||
            ...
            // =========================
            setProcessName(
                responseData?.process?.name || ""
            );

            const processData =
                responseData?.process_data || [];
            ...
            const hodReviewComments =
                getProcessValue(
                    processData,
                    "hod_review_comments"
                );

            const hodReviewAttachment =
                getProcessValue(
                    processData,
                    "hod_review_attachment"
                );

            const qaReviewComments =
                getProcessValue(
                    processData,
                    "qa_review_comments"
                );

            const qaReviewAttachment =
                getProcessValue(
                    processData,
                    "qa_review_attachment"
                );

            const qaApprovalComments =
                getProcessValue(
                    processData,
                    "qa_approval_comments"
                );

            const qaApprovalAttachment =
                getProcessValue(
                    processData,
            ...
            setDateOfInitiation(
                processDateOfInitiation ||
                    responseData?.initiation_date ||
                    ""
            );
            ...
            setCalibrationRows(
                normalizeGridRows(
                    responseData?.gridData ||
                        responseData?.grid_data ||
                        []
                )
            );
        } catch (error) {
            console.error(
                "Failed to fetch calibration detail:",
                error
            );
    ...
    useEffect(() => {
        ...
        const fetchStages = async () => {
            try {
                ...
                console.error(
                    "Failed to fetch workflow stages:",
                    error
                );

                toast.error(
                    error?.response?.data?.message ||
                        "Failed to load workflow stages."
                );
    ...
    onExit={() => navigate("/user/calibration-dashboard")}
/>
                    </div>
                </div>
            </div>
